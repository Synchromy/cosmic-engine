...
export async function runSetId(engine: BrainEngine, rawArgs: string[]): Promise<void> {
  ...
  // The old path is a POINTER, moved the way `sources set-path` moves it:
  // the database is updated, the directory on disk is not. The vault is a
  // render of the database (Cosmic #365), so the next reconcile writes the
  // new tree; touching files here would be a second writer.
  ...
  if (newPath !== src.local_path) console.log(`  local_path pointer: ${src.local_path} → ${newPath}`);
