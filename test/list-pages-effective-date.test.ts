 ...
 *
 * The cost of that gap was measured, not guessed: finding one tenant's
 ...
 * out when it was about (cosmic-hub#530).
 *
...
beforeAll(async () => {
  ...
  for (const [slug, offset] of [
    ...
    // The SAME meeting again, from the second calendar lane, under a title
    // that disagrees. cosmic-hub#549. Pinned here because a window over a
    // corpus holding each meeting twice must return it twice — see the test.
...
describe('list_pages effective_after / effective_before', () => {
  ...
  test('a thing the corpus holds twice comes back twice', async () => {
    // cosmic-hub#549: two calendar lanes carry the same meeting under
    // disagreeing titles. Collapsing them HERE would hide an ingestion fault
    // behind a read tool, so the window reports the corpus as it is.
